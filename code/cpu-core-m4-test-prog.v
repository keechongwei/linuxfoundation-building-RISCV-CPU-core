...
\TLV
   ...
   $next_pc[31:0] = $reset ? 0 : $taken_br ? $br_tgt_pc : $is_jal ? $br_tgt_pc : $is_jalr ? $jalr_tgt_pc  : $pc + 32'b100;
   $pc[31:0] = >>1$next_pc;
   ...
   $is_load = $opcode == 7'b0110111 || $opcode == 7'b0000011;
   //ALU Logic
   ...
   // Addressing Memory Logic
   $wr_en_mem = $is_s_instr ? 1 : 0 ;
   $rd_en = $is_load ? 1 : 0 ;
   ...
   m4+tb()
   m4+rf(32, 32, $reset, $wr_en, $rd[4:0], $is_load ? $ld_data : $result[31:0], $rs1_valid, $rs1[4:0], $src1_value, $rs2_valid, $rs2[4:0], $src2_value)
   
   m4+dmem(32, 32, $reset, $result[6:2], $wr_en_mem, $src2_value, $rd_en, $ld_data)
   m4+cpu_viz()
\SV
   endmodule